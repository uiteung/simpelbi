...
function deleteFile(id_formulir_spmi) {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      const apiUrlGetfileById = `https://simbe-dev.ulbi.ac.id/api/v1/dokumen/get?id_formulir_spmi=${id_formulir_spmi}`;
      ...
      CihuyDataAPI(apiUrlGetfileById, token, (error, response) => {
        if (error) {
          ...
          if (fileData) {
            ...
            const apiUrlfilesDelete = `https://simbe-dev.ulbi.ac.id/api/v1/dokumen/delete?id_formulir_spmi=${FileIDtoDelete}`;
